...
namespace TestApplication.Controllers
{
    public class InvoicesController : Controller
    {
        ...
        [HttpPost]
        public ActionResult FetchCostPerUnit(string productName)
        {
            if (!string.IsNullOrEmpty(productName))
            {
                var product = db.Products.Where(s => s.ID.ToString() == productName).FirstOrDefault<Product>();
                return Json(new { product.Cost }, JsonRequestBehavior.AllowGet );
            }
            return null;
        }
        ...
        public ActionResult Create(Invoices invoices)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isStockAvailable)
                {
                    ...
                    invoices.InvoiceTotal = ((invoices.CostPerUnit * invoices.Quantity) - invoices.Discount);
                    var tax = 0.04 * invoices.InvoiceTotal;
                    invoices.InvoiceTotal = invoices.InvoiceTotal + tax;
                    invoices.InvoiceTotal = ((invoices.CostPerUnit * invoices.Quantity)-invoices.Discount) + tax;
